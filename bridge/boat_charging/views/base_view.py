...
class BaseView(GenericAPIView):
    ...
    def __init__(self, **kwargs):
        ...
        self.location_status_kw_mapping = self.get_location_statuses_and_kw()
    ...
    def determine_overall_status_and_wattage(
        self, connectors: list[tuple[str, int]]
        ...
        """
        available = [kw for s, kw in connectors if s == "AVAILABLE"]
        occupied = [kw for s, kw in connectors if s == "OCCUPIED"]
        out_of_order = [
            kw for s, kw in connectors if s not in ("AVAILABLE", "OCCUPIED")
        ]
