...
from core.settings.base import ENVIRONMENT_SLUG


@pytest.mark.django_db
def test_user_groups_contains_group_names_positive():
    user = User.objects.create(username="testuser")
    group_name = f"{ENVIRONMENT_SLUG}-admin"
...
def test_user_groups_contains_group_names_negative():
    user = User.objects.create(username="testuser")
    Group.objects.create(name=f"{ENVIRONMENT_SLUG}-other")
...
def test_user_groups_contains_group_names_multiple():
    user = User.objects.create(username="testuser")
    group1 = Group.objects.create(name=f"{ENVIRONMENT_SLUG}-admin")
    group2 = Group.objects.create(name=f"{ENVIRONMENT_SLUG}-editor")
