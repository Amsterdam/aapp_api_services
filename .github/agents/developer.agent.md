...
- All code changes must be committed with a message that starts with "Developer: " followed by a concise description of the change. Openapi specs should not be added to a commit. So openapi.yaml changes should be ignored (as they are in .gitignore) and not included in the commit message. The openapi.yaml file is generated from the code and should not be manually edited. The openapi.yaml file is generated by running `SERVICE_NAME=<service_name> make spectacular` in the root of the repository.
