...
class TestSessionInitView(BaseCityPassTestCase):
    ...
    def test_session_init_with_existing_device_id(self):
        headers = {**self.headers, settings.HEADER_DEVICE_ID: "test-device-id"}
        result = self.client.post(self.api_url, headers=headers, follow=True)
        self.assertEqual(result.status_code, 200)
        result = self.client.post(self.api_url, headers=headers, follow=True)
    ...
    def test_session_init_after_cut_off_creates_a_new_usable_session(self):
        with freeze_time("2026-08-01 00:00:00+02:00"):
            init_result = self.client.post(
                self.api_url, headers=self.headers, follow=True
            )

        self.assertEqual(init_result.status_code, 200)

        access_token = init_result.data["access_token"]
        logout_headers = {**self.headers, settings.ACCESS_TOKEN_HEADER: access_token}

        with freeze_time("2026-08-01 00:00:01+02:00"):
            logout_result = self.client.post(
...
class TestSessionPostCityPassCredentialView(BaseCityPassTestCase):
    ...
    def setUp(self) -> None:
        ...
        self.headers[settings.SESSION_CREDENTIALS_KEY_HEADER] = mijn_ams_api_keys.split(
            ","
        )[0]
    ...
    def test_default_api_key_not_allowed(self):
        headers = self.headers
        headers[settings.SESSION_CREDENTIALS_KEY_HEADER] = self.headers[
