...
def get_serializer(status_code, exceptions):
    for exp in exceptions:
        assert status_code == exp.status_code, "All exceptions must have the same status code"

    hash_val = get_hash(exceptions)
    if hash_val in SERIALIZERS:
        return SERIALIZERS[hash_val]

    serializer = create_serializer(status_code=status_code, exceptions=exceptions, hash_val=hash_val)
...
def create_serializer(status_code, exceptions, hash_val):
    ...
    class ExceptionSerializer(serializers.Serializer):
        class Meta:
            ref_name = f"Status{status_code}_{hash_val}Serializer"  # Unique serializer name

        code = serializers.ChoiceField(choices=[
            (exp.default_code, exp.default_detail)
            for exp in exceptions
        ])
